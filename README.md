...
##### 安装方法
```shell
$ pip install redisbuffer
```

##### 缓存函数执行结果

```python
@redis_buffer.pickle_cache(key=lambda v, t: "cache:{}:{}".format(v, t), ex=180)
...
@redis_buffer.json_cache(key=lambda v, t: "cache:{}:{}".format(v, t), ex=180)
def hell_world(val: str, times: int):
...
class Demo:
    ...
    xxx = redis_buffer.property(key=lambda: "Demo::xxx", ex=10)(lambda self: self.load_xxx())
...
@redis_buffer.remove(lambda o: "auth::user:{}".format(o))
...
@redis_buffer.remove(lambda o: "auth::user:{}".format(o), by_return=True)
def modify_user(user_id):
    # DO MODIFY USER
    return "*********"
```
